...
for dyr in os.listdir('Agurim'):
    final_cols = ['filename', 'LAT', 'LON', 'cluster', 'cluster_size', 'Average_distance_to_neighbors', 'Average_distance_STD', 'Water_temperature']
        ...
        if 'leg' in f :
            ...
            for file in files : 
                if 'pix' in file :
                    ...
                    table_leg['x'], table_leg['x'] = coordinates[:,0], coordinates[:,1] 
    ...
    for cluster in centroids.keys():
        if shore_history[cluster] :
            ...
            plt.pause(1)
            plt.savefig(f"shore_distance_histogram_cluster_{cluster} date {dyr}.png", dpi=300, bbox_inches='tight' )
        else :
            plt.plot(0,0)
    ...
    print(date_table) 
    date_table.to_excel(f'Final_results.xlsx{dyr}')   
    #print (files)
    print ('--------------------------------')
